...
- Lightweight PSR-4 autoloader (`includes/autoload.php`) with no runtime Composer dependencies.
- `PluginBootstrap` and `PluginContext` for explicit service wiring.
- PHP templates (`templates/`) via `TemplateRenderer` (replaces Twig).
- Local Docker stack in `.wordpress/docker-compose.yml` with repository bind-mount.
- Playground blueprints: `.sandbox/blueprint.json`, root `blueprint.json`, updated `.plugin-assets/blueprints/blueprint.json`.

### Changed

- Removed `vendor/`, `vendor_prefixed/`, Symfony DI, and Twig from the distributed plugin.
- `composer.json` is dev/metadata only (PHP ^8.1).

### Fixed

- WordPress 6.7+ `_load_textdomain_just_in_time` notice (translations on `init`; lazy `WPR_Widget` and admin services).
