...
use WPR\Vendor\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException;
/**
...
class Definition
{
    ...
    public function setFactory($factory)
    {
        ...
        } elseif ($factory instanceof \WPR\Vendor\Symfony\Component\DependencyInjection\Reference) {
     ...
     */
    public function setDecoratedService(?string $id, ?string $renamedId = null, int $priority = 0, int $invalidBehavior = \WPR\Vendor\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE)
    {
        if ($renamedId && $id === $renamedId) {
            throw new \WPR\Vendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('The decorated service inner name for "%s" must be different than the service name itself.', $id));
        }
    ...
    public function replaceArgument($index, $argument)
    {
        if (0 === \count($this->arguments)) {
            throw new \WPR\Vendor\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException('Cannot replace arguments if none have been configured yet.');
        }
        if (\is_int($index) && ($index < 0 || $index > \count($this->arguments) - 1)) {
            throw new \WPR\Vendor\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException(\sprintf('The index "%d" is not in the range [0, %d].', $index, \count($this->arguments) - 1));
        }
        if (!\array_key_exists($index, $this->arguments)) {
            throw new \WPR\Vendor\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException(\sprintf('The argument "%s" doesn\'t exist.', $index));
        }
    ...
    {
        if (empty($method)) {
            throw new \WPR\Vendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('Method name cannot be empty.');
        }
    ...
    public function setDeprecated()
    {
        ...
        if ('' !== $message) {
            if (\preg_match('#[\\r\\n]|\\*/#', $message)) {
                throw new \WPR\Vendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('Invalid characters found in deprecation template.');
            }
            if (\false === \strpos($message, '%service_id%')) {
                throw new \WPR\Vendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('The deprecation template must contain the "%service_id%" placeholder.');
            }
        }
    ...
    public function setConfigurator($configurator)
    {
        ...
        } elseif ($configurator instanceof \WPR\Vendor\Symfony\Component\DependencyInjection\Reference) {
    ...
    public function setBindings(array $bindings)
    {
        foreach ($bindings as $key => $binding) {
            ...
            if (!$binding instanceof \WPR\Vendor\Symfony\Component\DependencyInjection\Argument\BoundArgument) {
                $bindings[$key] = new \WPR\Vendor\Symfony\Component\DependencyInjection\Argument\BoundArgument($binding);
            }
        }
