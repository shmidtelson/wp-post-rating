...
use WPR\Vendor\Symfony\Component\Config\Definition\Exception\ForbiddenOverwriteException;
use WPR\Vendor\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException;
use WPR\Vendor\Symfony\Component\Config\Definition\Exception\InvalidTypeException;
 ...
 */
abstract class BaseNode implements \WPR\Vendor\Symfony\Component\Config\Definition\NodeInterface
{
     ...
     */
    public function __construct(?string $name, \WPR\Vendor\Symfony\Component\Config\Definition\NodeInterface $parent = null, string $pathSeparator = self::DEFAULT_PATH_SEPARATOR)
    {
    ...
    public final function merge($leftSide, $rightSide)
    {
        if (!$this->allowOverwrite) {
            throw new \WPR\Vendor\Symfony\Component\Config\Definition\Exception\ForbiddenOverwriteException(\sprintf('Configuration path "%s" cannot be overwritten. You have to define all options for this path, and any of its sub-paths in one configuration section.', $this->getPath()));
        }
    ...
    public final function finalize($value)
    {
        ...
        foreach ($this->finalValidationClosures as $closure) {
            try {
                $value = $closure($value);
            } catch (\WPR\Vendor\Symfony\Component\Config\Definition\Exception\Exception $e) {
                if ($e instanceof \WPR\Vendor\Symfony\Component\Config\Definition\Exception\UnsetKeyException && null !== $this->handlingPlaceholder) {
                    continue;
                }
                throw $e;
            } catch (\Exception $e) {
                throw new \WPR\Vendor\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException(\sprintf('Invalid configuration for path "%s": ', $this->getPath()) . $e->getMessage(), $e->getCode(), $e);
            }
        }
        return $value;
    }
    ...
    private function doValidateType($value) : void
    {
        if (null !== $this->handlingPlaceholder && !$this->allowPlaceholders()) {
            $e = new \WPR\Vendor\Symfony\Component\Config\Definition\Exception\InvalidTypeException(\sprintf('A dynamic value is not compatible with a "%s" node type at path "%s".', static::class, $this->getPath()));
        ...
        if ($validTypes && \array_diff($knownTypes, $validTypes)) {
            $e = new \WPR\Vendor\Symfony\Component\Config\Definition\Exception\InvalidTypeException(\sprintf('Invalid type for path "%s". Expected %s, but got %s.', $this->getPath(), 1 === \count($validTypes) ? '"' . \reset($validTypes) . '"' : 'one of "' . \implode('", "', $validTypes) . '"', 1 === \count($knownTypes) ? '"' . \reset($knownTypes) . '"' : 'one of "' . \implode('", "', $knownTypes) . '"'));
