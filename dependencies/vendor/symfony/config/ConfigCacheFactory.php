 ...
 */
class ConfigCacheFactory implements \WPR\Vendor\Symfony\Component\Config\ConfigCacheFactoryInterface
{
    private $debug;
    /**
    ...
    {
        $cache = new \WPR\Vendor\Symfony\Component\Config\ConfigCache($file, $this->debug);
        if (!$cache->isFresh()) {
            $callback($cache);
        }
        return $cache;
