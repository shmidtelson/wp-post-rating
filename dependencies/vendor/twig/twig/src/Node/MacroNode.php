 ...
 */
class MacroNode extends \WPR\Vendor\Twig\Node\Node
{
    const VARARGS_NAME = 'varargs';
    public function __construct(string $name, \WPR\Vendor\Twig\Node\Node $body, \WPR\Vendor\Twig\Node\Node $arguments, int $lineno, string $tag = null)
    {
        foreach ($arguments as $argumentName => $argument) {
            if (self::VARARGS_NAME === $argumentName) {
                throw new \WPR\Vendor\Twig\Error\SyntaxError(\sprintf('The argument "%s" in macro "%s" cannot be defined because the variable "%s" is reserved for arbitrary arguments.', self::VARARGS_NAME, $name, self::VARARGS_NAME), $argument->getTemplateLine(), $argument->getSourceContext());
            }
        }
