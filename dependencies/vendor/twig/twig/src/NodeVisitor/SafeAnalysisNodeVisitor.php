...
final class SafeAnalysisNodeVisitor implements \WPR\Vendor\Twig\NodeVisitor\NodeVisitorInterface
{
    private $data = [];
    private $safeVars = [];
    ...
    public function getSafe(\WPR\Vendor\Twig\Node\Node $node)
    {
    ...
    private function setSafe(\WPR\Vendor\Twig\Node\Node $node, array $safe) : void
    {
        $hash = \spl_object_hash($node);
        if (isset($this->data[$hash])) {
            foreach ($this->data[$hash] as &$bucket) {
    ...
    public function leaveNode(\WPR\Vendor\Twig\Node\Node $node, \WPR\Vendor\Twig\Environment $env) : ?\WPR\Vendor\Twig\Node\Node
    {
        if ($node instanceof \WPR\Vendor\Twig\Node\Expression\ConstantExpression) {
            ...
            $this->setSafe($node, ['all']);
        } elseif ($node instanceof \WPR\Vendor\Twig\Node\Expression\ParentExpression) {
            // parent block is safe by definition
            $this->setSafe($node, ['all']);
        } elseif ($node instanceof \WPR\Vendor\Twig\Node\Expression\ConditionalExpression) {
        ...
        } elseif ($node instanceof \WPR\Vendor\Twig\Node\Expression\FunctionExpression) {
        ...
        } elseif ($node instanceof \WPR\Vendor\Twig\Node\Expression\GetAttrExpression && $node->getNode('node') instanceof \WPR\Vendor\Twig\Node\Expression\NameExpression) {
