 ...
 */
abstract class IntegrationTestCase extends \WPR\Vendor\PHPUnit\Framework\TestCase
{
    /**
     * @return string
     */
    ...
    protected function doIntegrationTest($file, $message, $condition, $templates, $exception, $outputs, $deprecation = '')
    {
        ...
        $loader = new \WPR\Vendor\Twig\Loader\ArrayLoader($templates);
        foreach ($outputs as $i => $match) {
            $config = \array_merge(['cache' => \false, 'strict_variables' => \true], $match[2] ? eval($match[2] . ';') : []);
            $twig = new \WPR\Vendor\Twig\Environment($loader, $config);
            ...
            try {
                ...
                throw new \WPR\Vendor\Twig\Error\Error(\sprintf('%s: %s', \get_class($e), $e->getMessage()), -1, null, $e);
            } finally {
            ...
            try {
                ...
                $e = new \WPR\Vendor\Twig\Error\Error(\sprintf('%s: %s', \get_class($e), $e->getMessage()), -1, null, $e);
                $output = \trim(\sprintf('%s: %s', \get_class($e), $e->getMessage()));
            }
            if (\false !== $exception) {
                list($class) = \explode(':', $exception);
                $constraintClass = \class_exists('WPR\\Vendor\\PHPUnit\\Framework\\Constraint\\Exception') ? 'PHPUnit\\Framework\\Constraint\\Exception' : 'PHPUnit_Framework_Constraint_Exception';
