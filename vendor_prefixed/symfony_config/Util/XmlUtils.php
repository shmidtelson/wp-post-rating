 ...
 */
namespace WPR_Vendor\Symfony\Component\Config\Util;

use WPR_Vendor\Symfony\Component\Config\Util\Exception\InvalidXmlException;
use WPR_Vendor\Symfony\Component\Config\Util\Exception\XmlParsingException;
/**
...
class XmlUtils
{
    ...
    public static function parse(string $content, $schemaOrCallable = null)
    {
        ...
        if (!$dom->loadXML($content, \LIBXML_NONET | (\defined('LIBXML_COMPACT') ? \LIBXML_COMPACT : 0))) {
            ...
            throw new \WPR_Vendor\Symfony\Component\Config\Util\Exception\XmlParsingException(\implode("\n", static::getXmlErrors($internalErrors)));
        }
        ...
        foreach ($dom->childNodes as $child) {
            if (\XML_DOCUMENT_TYPE_NODE === $child->nodeType) {
                throw new \WPR_Vendor\Symfony\Component\Config\Util\Exception\XmlParsingException('Document types are not allowed.');
            }
        }
        if (null !== $schemaOrCallable) {
            ...
            if (\is_callable($schemaOrCallable)) {
                ...
                \libxml_use_internal_errors($internalErrors);
                throw new \WPR_Vendor\Symfony\Component\Config\Util\Exception\XmlParsingException('The schemaOrCallable argument has to be a valid path to XSD file or callable.');
            }
            if (!$valid) {
                $messages = static::getXmlErrors($internalErrors);
                if (empty($messages)) {
                    throw new \WPR_Vendor\Symfony\Component\Config\Util\Exception\InvalidXmlException('The XML is not valid.', 0, $e);
                }
                throw new \WPR_Vendor\Symfony\Component\Config\Util\Exception\XmlParsingException(\implode("\n", $messages), 0, $e);
            }
        }
    ...
    public static function loadFile(string $file, $schemaOrCallable = null)
    {
        ...
        } catch (\WPR_Vendor\Symfony\Component\Config\Util\Exception\InvalidXmlException $e) {
            throw new \WPR_Vendor\Symfony\Component\Config\Util\Exception\XmlParsingException(\sprintf('The XML file "%s" is not valid.', $file), 0, $e->getPrevious());
        }
    }
    /**
