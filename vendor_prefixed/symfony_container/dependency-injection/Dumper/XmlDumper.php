 ...
 */
class XmlDumper extends \WPR_Vendor\Symfony\Component\DependencyInjection\Dumper\Dumper
{
    /**
     * @var \DOMDocument
     */
    ...
    private function addService(\WPR_Vendor\Symfony\Component\DependencyInjection\Definition $definition, ?string $id, \DOMElement $parent)
    {
        ...
        if (null !== ($decoratedService = $definition->getDecoratedService())) {
            ...
            if (\in_array($decorationOnInvalid, [\WPR_Vendor\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_INVALID_REFERENCE, \WPR_Vendor\Symfony\Component\DependencyInjection\ContainerInterface::NULL_ON_INVALID_REFERENCE], \true)) {
        ...
        if ($callable = $definition->getFactory()) {
            ...
            if (\is_array($callable) && $callable[0] instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Definition) {
                $this->addService($callable[0], null, $factory);
                $factory->setAttribute('method', $callable[1]);
            } elseif (\is_array($callable)) {
                if (null !== $callable[0]) {
                    $factory->setAttribute($callable[0] instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Reference ? 'service' : 'class', $callable[0]);
                }
                $factory->setAttribute('method', $callable[1]);
            } else {
        ...
        if ($callable = $definition->getConfigurator()) {
            ...
            if (\is_array($callable) && $callable[0] instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Definition) {
                ...
                $configurator->setAttribute($callable[0] instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Reference ? 'service' : 'class', $callable[0]);
                $configurator->setAttribute('method', $callable[1]);
            } else {
    ...
    private function addServiceAlias(string $alias, \WPR_Vendor\Symfony\Component\DependencyInjection\Alias $id, \DOMElement $parent)
    {
        $service = $this->document->createElement('service');
    ...
    private function convertParameters(array $parameters, string $type, \DOMElement $parent, string $keyAttribute = 'key')
    {
        ...
        foreach ($parameters as $key => $value) {
            ...
            if (\is_array($tag = $value)) {
            ...
            } elseif ($value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument || $value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument && ($tag = $value->getTaggedIteratorArgument())) {
                $element->setAttribute('type', $value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument ? 'tagged_iterator' : 'tagged_locator');
                ...
                if (\WPR_Vendor\Symfony\Component\DependencyInjection\ContainerInterface::NULL_ON_INVALID_REFERENCE == $behavior) {
                    $element->setAttribute('on-invalid', 'null');
                } elseif (\WPR_Vendor\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_INVALID_REFERENCE == $behavior) {
                    $element->setAttribute('on-invalid', 'ignore');
                } elseif (\WPR_Vendor\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE == $behavior) {
