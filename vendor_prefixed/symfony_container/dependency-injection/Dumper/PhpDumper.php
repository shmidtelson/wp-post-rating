...
use WPR_Vendor\Composer\Autoload\ClassLoader;
use WPR_Vendor\Symfony\Component\Debug\DebugClassLoader as LegacyDebugClassLoader;
...
use WPR_Vendor\Symfony\Component\DependencyInjection\Argument\ServiceLocator;
use WPR_Vendor\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument;
use WPR_Vendor\Symfony\Component\DependencyInjection\Compiler\AnalyzeServiceReferencesPass;
use WPR_Vendor\Symfony\Component\DependencyInjection\Compiler\CheckCircularReferencesPass;
use WPR_Vendor\Symfony\Component\DependencyInjection\Compiler\ServiceReferenceGraphEdge;
use WPR_Vendor\Symfony\Component\DependencyInjection\Compiler\ServiceReferenceGraphNode;
use WPR_Vendor\Symfony\Component\DependencyInjection\Container;
...
use WPR_Vendor\Symfony\Component\DependencyInjection\Exception\EnvParameterException;
...
use WPR_Vendor\Symfony\Component\DependencyInjection\Exception\ServiceCircularReferenceException;
use WPR_Vendor\Symfony\Component\DependencyInjection\ExpressionLanguage;
use WPR_Vendor\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\DumperInterface as ProxyDumper;
use WPR_Vendor\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper;
use WPR_Vendor\Symfony\Component\DependencyInjection\Loader\FileLoader;
...
use WPR_Vendor\Symfony\Component\DependencyInjection\ServiceLocator as BaseServiceLocator;
use WPR_Vendor\Symfony\Component\DependencyInjection\TypedReference;
use WPR_Vendor\Symfony\Component\DependencyInjection\Variable;
use WPR_Vendor\Symfony\Component\ErrorHandler\DebugClassLoader;
use WPR_Vendor\Symfony\Component\ExpressionLanguage\Expression;
use WPR_Vendor\Symfony\Component\HttpKernel\Kernel;
/**
 ...
 */
class PhpDumper extends \WPR_Vendor\Symfony\Component\DependencyInjection\Dumper\Dumper
{
    /**
    ...
    public function __construct(\WPR_Vendor\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        if (!$container->isCompiled()) {
            throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\LogicException('Cannot dump an uncompiled container.');
        }
     ...
     */
    public function setProxyDumper(\WPR_Vendor\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\DumperInterface $proxyDumper)
    {
    ...
    public function dump(array $options = [])
    {
        ...
        if (0 !== \strpos($baseClass = $options['base_class'], '\\') && 'Container' !== $baseClass) {
            ...
            $this->baseClass = \WPR_Vendor\Symfony\Component\DependencyInjection\Container::class;
        } else {
            $this->baseClass = $baseClass;
        }
        $this->initializeMethodNamesMap('Container' === $baseClass ? \WPR_Vendor\Symfony\Component\DependencyInjection\Container::class : $baseClass);
        if ($this->getProxyDumper() instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper) {
            (new \WPR_Vendor\Symfony\Component\DependencyInjection\Compiler\AnalyzeServiceReferencesPass(\true, \false))->process($this->container);
            try {
                (new \WPR_Vendor\Symfony\Component\DependencyInjection\Compiler\CheckCircularReferencesPass())->process($this->container);
            } catch (\WPR_Vendor\Symfony\Component\DependencyInjection\Exception\ServiceCircularReferenceException $e) {
                $path = $e->getPath();
                \end($path);
                $path[\key($path)] .= '". Try running "composer require symfony/proxy-manager-bridge';
                throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\ServiceCircularReferenceException($e->getServiceId(), $path);
            }
        }
        (new \WPR_Vendor\Symfony\Component\DependencyInjection\Compiler\AnalyzeServiceReferencesPass(\false, !$this->getProxyDumper() instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper))->process($this->container);
        $checkedNodes = [];
        $this->circularReferences = [];
        $this->singleUsePrivateIds = [];
        foreach ($this->container->getCompiler()->getServiceReferenceGraph()->getNodes() as $id => $node) {
            if (!$node->getValue() instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Definition) {
                continue;
            }
...
use WPR_Vendor\\Symfony\\Component\\DependencyInjection\\Argument\\RewindableGenerator;
use WPR_Vendor\\Symfony\\Component\\DependencyInjection\\Exception\\RuntimeException;
...
EOF;
            ...
            $hash = \ucfirst(\strtr(\WPR_Vendor\Symfony\Component\DependencyInjection\ContainerBuilder::hash($files), '._', 'xx'));
            $code = [];
...
use WPR_Vendor\\Symfony\\Component\\DependencyInjection\\Dumper\\Preloader;
...
EOF;
        ...
        if ($unusedEnvs) {
            throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\EnvParameterException($unusedEnvs, null, 'Environment variables "%s" are never used. Please, check your container\'s configuration.');
        }
        return $code;
    }
    /**
     ...
     */
    private function getProxyDumper() : \WPR_Vendor\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\DumperInterface
    {
        if (!$this->proxyDumper) {
            $this->proxyDumper = new \WPR_Vendor\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper();
        }
    ...
    private function analyzeCircularReferences(string $sourceId, array $edges, array &$checkedNodes, array &$currentPath = [], bool $byConstructor = \true)
    {
        ...
        foreach ($edges as $edge) {
            ...
            $id = $node->getId();
            if (!$node->getValue() instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Definition || $sourceId === $id || $edge->isLazy() || $edge->isWeak()) {
                // no-op
    ...
    private function generateProxyClasses() : array
    {
        ...
        $strip = '' === $this->docStar && \method_exists('WPR_Vendor\\Symfony\\Component\\HttpKernel\\Kernel', 'stripComments');
        ...
        foreach ($definitions as $definition) {
            ...
            if ($strip) {
                $proxyCode = "<?php\n" . $proxyCode;
                $proxyCode = \substr(\WPR_Vendor\Symfony\Component\HttpKernel\Kernel::stripComments($proxyCode), 5);
            }
    ...
    private function addServiceInclude(string $cId, \WPR_Vendor\Symfony\Component\DependencyInjection\Definition $definition) : string
    {
        $code = '';
        if ($this->inlineRequires && (!$this->isHotPath($definition) || $this->getProxyDumper()->isProxyCandidate($definition))) {
            ...
            foreach ($this->serviceCalls as $id => list($callCount, $behavior)) {
                if ('service_container' !== $id && $id !== $cId && \WPR_Vendor\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE !== $behavior && $this->container->has($id) && $this->isTrivialInstance($def = $this->container->findDefinition($id))) {
                    foreach ($this->getClasses($def, $cId) as $class) {
                        $this->collectLineage($class, $lineage);
                    }
                }
     ...
     */
    private function addServiceInstance(string $id, \WPR_Vendor\Symfony\Component\DependencyInjection\Definition $definition, bool $isSimpleInstance) : string
    {
        ...
        if (0 === \strpos($class, "'") && \false === \strpos($class, '$') && !\preg_match('/^\'(?:\\\\{2})?[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*(?:\\\\{2}[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*)*\'$/', $class)) {
            throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('"%s" is not a valid class name for the "%s" service.', $class, $id));
        }
    ...
    private function isTrivialInstance(\WPR_Vendor\Symfony\Component\DependencyInjection\Definition $definition) : bool
    {
        ...
        foreach ($definition->getArguments() as $arg) {
            if (!$arg || $arg instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Parameter) {
                continue;
            }
            if (\is_array($arg) && 3 >= \count($arg)) {
                foreach ($arg as $k => $v) {
                    ...
                    if (!$v || $v instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Parameter) {
                        continue;
                    }
                    if ($v instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Reference && $this->container->has($id = (string) $v) && $this->container->findDefinition($id)->isSynthetic()) {
                        continue;
                    }
            ...
            } elseif ($arg instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Reference && $this->container->has($id = (string) $arg) && $this->container->findDefinition($id)->isSynthetic()) {
                continue;
    ...
    private function addServiceMethodCalls(\WPR_Vendor\Symfony\Component\DependencyInjection\Definition $definition, string $variableName, ?string $sharedNonLazyId) : string
    {
        $lastWitherIndex = null;
        foreach ($definition->getMethodCalls() as $k => $call) {
            if ($call[2] ?? \false) {
    ...
    private function addServiceProperties(\WPR_Vendor\Symfony\Component\DependencyInjection\Definition $definition, string $variableName = 'instance') : string
    {
        $code = '';
    ...
    private function addServiceConfigurator(\WPR_Vendor\Symfony\Component\DependencyInjection\Definition $definition, string $variableName = 'instance') : string
    {
        if (!($callable = $definition->getConfigurator())) {
            return '';
        }
        if (\is_array($callable)) {
            if ($callable[0] instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Reference || $callable[0] instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Definition && $this->definitionVariables->contains($callable[0])) {
    ...
    private function addService(string $id, \WPR_Vendor\Symfony\Component\DependencyInjection\Definition $definition) : array
    {
        $this->definitionVariables = new \SplObjectStorage();
        $this->referenceVariables = [];
        $this->variableCount = 0;
        $this->referenceVariables[$id] = new \WPR_Vendor\Symfony\Component\DependencyInjection\Variable('instance');
        $return = [];
        if ($class = $definition->getClass()) {
            $class = $class instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Parameter ? '%' . $class . '%' : $this->container->resolveEnvPlaceholders($class);
            $return[] = \sprintf(0 === \strpos($class, '%') ? '@return object A %1$s instance' : '@return \\%s', \ltrim($class, '\\'));
        } elseif ($definition->getFactory()) {
            $factory = $definition->getFactory();
            if (\is_string($factory)) {
                $return[] = \sprintf('@return object An instance returned by %s()', $factory);
            } elseif (\is_array($factory) && (\is_string($factory[0]) || $factory[0] instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Definition || $factory[0] instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Reference)) {
                $class = $factory[0] instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Definition ? $factory[0]->getClass() : (string) $factory[0];
                $class = $class instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Parameter ? '%' . $class . '%' : $this->container->resolveEnvPlaceholders($class);
...
EOF;
    ...
    private function addInlineVariables(string $id, \WPR_Vendor\Symfony\Component\DependencyInjection\Definition $definition, array $arguments, bool $forConstructor) : string
    {
        $code = '';
        foreach ($arguments as $argument) {
            if (\is_array($argument)) {
                $code .= $this->addInlineVariables($id, $definition, $argument, $forConstructor);
            } elseif ($argument instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Reference) {
                $code .= $this->addInlineReference($id, $definition, $argument, $forConstructor);
            } elseif ($argument instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Definition) {
                $code .= $this->addInlineService($id, $definition, $argument, $forConstructor);
            }
        }
        return $code;
    }
    private function addInlineReference(string $id, \WPR_Vendor\Symfony\Component\DependencyInjection\Definition $definition, string $targetId, bool $forConstructor) : string
    {
        ...
        $this->referenceVariables[$targetId] = new \WPR_Vendor\Symfony\Component\DependencyInjection\Variable($name);
        $reference = \WPR_Vendor\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE >= $behavior ? new \WPR_Vendor\Symfony\Component\DependencyInjection\Reference($targetId, $behavior) : null;
...
, $this->container->getDefinition($id)->isPublic() ? 'services' : 'privates', $this->doExport($id));
    ...
    private function addInlineService(string $id, \WPR_Vendor\Symfony\Component\DependencyInjection\Definition $definition, \WPR_Vendor\Symfony\Component\DependencyInjection\Definition $inlineDef = null, bool $forConstructor = \true) : string
    {
        $code = '';
        ...
        if (isset($this->definitionVariables[$inlineDef])) {
            ...
            $this->definitionVariables[$inlineDef] = new \WPR_Vendor\Symfony\Component\DependencyInjection\Variable($name);
            $code .= '' !== $code ? "\n" : '';
            if ('instance' === $name) {
    ...
    private function addNewInstance(\WPR_Vendor\Symfony\Component\DependencyInjection\Definition $definition, string $return = '', string $id = null) : string
    {
        $tail = $return ? ";\n" : '';
        if (\WPR_Vendor\Symfony\Component\DependencyInjection\ServiceLocator::class === $definition->getClass() && $definition->hasTag($this->serviceLocatorTag)) {
            $arguments = [];
            foreach ($definition->getArgument(0) as $k => $argument) {
                $arguments[$k] = $argument->getValues()[0];
            }
            return $return . $this->dumpValue(new \WPR_Vendor\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument($arguments)) . $tail;
        }
        $arguments = [];
        ...
        if (null !== $definition->getFactory()) {
            ...
            if (\is_array($callable)) {
                if (!\preg_match('/^[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*$/', $callable[1])) {
                    throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition because of invalid factory method (%s).', $callable[1] ?: 'n/a'));
                }
                if ($callable[0] instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Reference || $callable[0] instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Definition && $this->definitionVariables->contains($callable[0])) {
                ...
                if (0 === \strpos($class, "'") && \false === \strpos($class, '$')) {
                    if ("''" === $class) {
                        throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition: "%s" service is defined to be created by a factory but is missing the service reference, did you forget to define the factory service id or class?', $id ? 'The "' . $id . '"' : 'inline'));
                    }
        ...
        if (null === ($class = $definition->getClass())) {
            throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have no class nor factory.');
        }
...
use WPR_Vendor\\Symfony\\Component\\DependencyInjection\\Exception\\LogicException;
use WPR_Vendor\\Symfony\\Component\\DependencyInjection\\Exception\\RuntimeException;
use WPR_Vendor\\Symfony\\Component\\DependencyInjection\\ParameterBag\\FrozenParameterBag;
...
EOF;
        ...
        if (\WPR_Vendor\Symfony\Component\DependencyInjection\Container::class !== $this->baseClass) {
            $r = $this->container->getReflectionClass($this->baseClass, \false);
            if (null !== $r && null !== ($constructor = $r->getConstructor()) && 0 === $constructor->getNumberOfRequiredParameters() && \WPR_Vendor\Symfony\Component\DependencyInjection\Container::class !== $constructor->getDeclaringClass()->name) {
...
EOF;
    ...
    private function addRemovedIds() : string
    {
        ...
        if ($this->asFiles) {
            ...
            \sort($ids);
            foreach ($ids as $id) {
                if (\preg_match(\WPR_Vendor\Symfony\Component\DependencyInjection\Loader\FileLoader::ANONYMOUS_ID_REGEXP, $id)) {
                    continue;
                }
                $code .= '            ' . $this->doExport($id) . " => true,\n";
            }
    ...
    private function addDefaultParametersMethod() : string
    {
        ...
        foreach ($this->container->getParameterBag()->all() as $key => $value) {
            if ($key !== ($resolvedKey = $this->container->resolveEnvPlaceholders($key))) {
                throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Parameter name cannot use env parameters: "%s".', $resolvedKey));
            }
...
EOF;
    ...
    private function exportParameters(array $parameters, string $path = '', int $indent = 12) : string
    {
        ...
        foreach ($parameters as $key => $value) {
            ...
            } elseif ($value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Argument\ArgumentInterface) {
                throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain special arguments. "%s" found in "%s".', \get_debug_type($value), $path . '/' . $key));
            } elseif ($value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Variable) {
                throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain variable references. Variable "%s" found in "%s".', $value, $path . '/' . $key));
            } elseif ($value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Definition) {
                throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain service definitions. Definition for "%s" found in "%s".', $value->getClass(), $path . '/' . $key));
            } elseif ($value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Reference) {
                throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain references to other services (reference to service "%s" found in "%s").', $value, $path . '/' . $key));
            } elseif ($value instanceof \WPR_Vendor\Symfony\Component\ExpressionLanguage\Expression) {
                throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain expressions. Expression "%s" found in "%s".', $value, $path . '/' . $key));
            } else {
...
EOF;
    ...
    {
        $conditions = [];
        foreach (\WPR_Vendor\Symfony\Component\DependencyInjection\ContainerBuilder::getInitializedConditionals($value) as $service) {
        ...
        foreach (\WPR_Vendor\Symfony\Component\DependencyInjection\ContainerBuilder::getServiceConditionals($value) as $service) {
    ...
    private function dumpValue($value, bool $interpolate = \true) : string
    {
        if (\is_array($value)) {
            ...
            try {
                if ($value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Argument\ServiceClosureArgument) {
                    $value = $value->getValues()[0];
                    $code = $this->dumpValue($value, $interpolate);
                    $returnedType = '';
                    if ($value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\TypedReference) {
                        $returnedType = \sprintf(': %s\\%s', \WPR_Vendor\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE >= $value->getInvalidBehavior() ? '' : '?', $value->getType());
                    }
                    $code = \sprintf('return %s;', $code);
                    return \sprintf("function ()%s {\n            %s\n        }", $returnedType, $code);
                }
                if ($value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Argument\IteratorArgument) {
                    $operands = [0];
                    $code = [];
                ...
                if ($value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument) {
                    ...
                    foreach ($value->getValues() as $k => $v) {
                        ...
                        $serviceMap .= \sprintf("\n            %s => [%s, %s, %s, %s],", $this->export($k), $this->export($definition->isShared() ? $definition->isPublic() ? 'services' : 'privates' : \false), $this->doExport($id), $this->export(\WPR_Vendor\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE !== $v->getInvalidBehavior() && !\is_string($load) ? $this->generateMethodName($id) : null), $this->export($load));
                        $serviceTypes .= \sprintf("\n            %s => %s,", $this->export($k), $this->export($v instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\TypedReference ? $v->getType() : '?'));
                        $this->locatedIds[$id] = \true;
                    }
                    $this->addGetService = \true;
                    return \sprintf('new \\%s($this->getService, [%s%s], [%s%s])', \WPR_Vendor\Symfony\Component\DependencyInjection\Argument\ServiceLocator::class, $serviceMap, $serviceMap ? "\n        " : '', $serviceTypes, $serviceTypes ? "\n        " : '');
                }
            } finally {
            ...
            if ($value->getMethodCalls()) {
                throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have method calls.');
            }
            if ($value->getProperties()) {
                throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have properties.');
            }
            if (null !== $value->getConfigurator()) {
                throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have a configurator.');
            }
        ...
        } elseif ($value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Parameter) {
        ...
        } elseif ($value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Argument\AbstractArgument) {
            throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\RuntimeException($value->getTextWithContext());
        } elseif (\is_object($value) || \is_resource($value)) {
            throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\RuntimeException('Unable to dump a service container if a parameter is an object or a resource.');
        }
    ...
    private function dumpLiteralClass(string $class) : string
    {
        ...
        if (0 !== \strpos($class, "'") || !\preg_match('/^\'(?:\\\\{2})?[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*(?:\\\\{2}[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*)*\'$/', $class)) {
            throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition because of invalid class name (%s).', $class ?: 'n/a'));
        }
    ...
    private function getServiceCall(string $id, \WPR_Vendor\Symfony\Component\DependencyInjection\Reference $reference = null) : string
    {
        while ($this->container->hasAlias($id)) {
            $id = (string) $this->container->getAlias($id);
        }
        ...
        if ($this->container->hasDefinition($id) && ($definition = $this->container->getDefinition($id))) {
            ...
            } elseif (null !== $reference && \WPR_Vendor\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE === $reference->getInvalidBehavior()) {
                $code = 'null';
                if (!$definition->isShared()) {
                    return $code;
                }
        ...
        if (null !== $reference && \WPR_Vendor\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE === $reference->getInvalidBehavior()) {
            return 'null';
        }
        if (null !== $reference && \WPR_Vendor\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE < $reference->getInvalidBehavior()) {
            $code = \sprintf('$this->get(%s, /* ContainerInterface::NULL_ON_INVALID_REFERENCE */ %d)', $this->doExport($id), \WPR_Vendor\Symfony\Component\DependencyInjection\ContainerInterface::NULL_ON_INVALID_REFERENCE);
        } else {
    ...
    private function generateMethodName(string $id) : string
    {
        ...
        $i = \strrpos($id, '\\');
        $name = \WPR_Vendor\Symfony\Component\DependencyInjection\Container::camelize(\false !== $i && isset($id[1 + $i]) ? \substr($id, 1 + $i) : $id);
    ...
    private function getExpressionLanguage() : \WPR_Vendor\Symfony\Component\DependencyInjection\ExpressionLanguage
    {
        if (null === $this->expressionLanguage) {
            if (!\class_exists('WPR_Vendor\\Symfony\\Component\\ExpressionLanguage\\ExpressionLanguage')) {
                throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\LogicException('Unable to use expressions as the Symfony ExpressionLanguage component is not installed.');
            }
            $providers = $this->container->getExpressionLanguageProviders();
            $this->expressionLanguage = new \WPR_Vendor\Symfony\Component\DependencyInjection\ExpressionLanguage(null, $providers, function ($arg) {
    ...
    private function isHotPath(\WPR_Vendor\Symfony\Component\DependencyInjection\Definition $definition) : bool
    {
        return $this->hotPathTag && $definition->hasTag($this->hotPathTag) && !$definition->isDeprecated();
    }
    private function isSingleUsePrivateNode(\WPR_Vendor\Symfony\Component\DependencyInjection\Compiler\ServiceReferenceGraphNode $node) : bool
    {
        ...
        foreach ($node->getInEdges() as $edge) {
            ...
            if ($edge->isLazy() || !$value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Definition || !$value->isShared()) {
                return \false;
            }
    ...
    private function getAutoloadFile() : ?string
    {
        ...
        foreach (\spl_autoload_functions() as $autoloader) {
            ...
            if ($autoloader[0] instanceof \WPR_Vendor\Symfony\Component\ErrorHandler\DebugClassLoader || $autoloader[0] instanceof \WPR_Vendor\Symfony\Component\Debug\DebugClassLoader) {
                $autoloader = $autoloader[0]->getClassLoader();
            }
            if (!\is_array($autoloader) || !$autoloader[0] instanceof \WPR_Vendor\Composer\Autoload\ClassLoader || !$autoloader[0]->findFile(__CLASS__)) {
                continue;
            }
    ...
    private function getClasses(\WPR_Vendor\Symfony\Component\DependencyInjection\Definition $definition, string $id) : array
    {
        $classes = [];
        while ($definition instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Definition) {
            foreach ($definition->getTag($this->preloadTags[0]) as $tag) {
                if (!isset($tag['class'])) {
                    throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Missing attribute "class" on tag "%s" for service "%s".', $this->preloadTags[0], $id));
                }
