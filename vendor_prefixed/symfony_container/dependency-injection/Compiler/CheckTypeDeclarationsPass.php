...
use WPR_Vendor\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException;
 ...
 */
final class CheckTypeDeclarationsPass extends \WPR_Vendor\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    protected function processValue($value, $isRoot = \false)
    {
        ...
        if (\WPR_Vendor\Symfony\Component\DependencyInjection\ServiceLocator::class === $value->getClass()) {
            return parent::processValue($value, $isRoot);
        }
     ...
     */
    private function checkTypeDeclarations(\WPR_Vendor\Symfony\Component\DependencyInjection\Definition $checkedDefinition, \ReflectionFunctionAbstract $reflectionFunction, array $values) : void
    {
        $numberOfRequiredParameters = $reflectionFunction->getNumberOfRequiredParameters();
        if (\count($values) < $numberOfRequiredParameters) {
            throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid definition for service "%s": "%s::%s()" requires %d arguments, %d passed.', $this->currentId, $reflectionFunction->class, $reflectionFunction->name, $numberOfRequiredParameters, \count($values)));
        }
        $reflectionParameters = $reflectionFunction->getParameters();
        $checksCount = \min($reflectionFunction->getNumberOfParameters(), \count($values));
        $envPlaceholderUniquePrefix = $this->container->getParameterBag() instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\ParameterBag\EnvPlaceholderParameterBag ? $this->container->getParameterBag()->getEnvPlaceholderUniquePrefix() : null;
     ...
     */
    private function checkType(\WPR_Vendor\Symfony\Component\DependencyInjection\Definition $checkedDefinition, $value, \ReflectionParameter $parameter, ?string $envPlaceholderUniquePrefix, string $type = null) : void
    {
        if (null === $type) {
            ...
            if ($type instanceof \ReflectionUnionType) {
                foreach ($type->getTypes() as $type) {
                    try {
                        ...
                        return;
                    } catch (\WPR_Vendor\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException $e) {
                    }
                }
                throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException($this->currentId, $e->getCode(), $parameter);
            }
            $type = $type->getName();
        }
        if ($value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Reference) {
            if (!$this->container->has($value = (string) $value)) {
                return;
            }
            if ('service_container' === $value && \is_a($type, \WPR_Vendor\Symfony\Component\DependencyInjection\Container::class, \true)) {
                return;
            }
        ...
        if (null === $class) {
            if ($value instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Argument\IteratorArgument) {
                $class = \WPR_Vendor\Symfony\Component\DependencyInjection\Argument\RewindableGenerator::class;
                ...
                $class = \WPR_Vendor\Symfony\Component\DependencyInjection\ServiceLocator::class;
        ...
        if ('callable' === $type && \is_array($value) && isset($value[0]) && ($value[0] instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Reference || $value[0] instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Definition || \is_string($value[0]))) {
            return;
        }
        ...
        if (!$parameter->getType()->isBuiltin() || !$checkFunction($value)) {
            throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException($this->currentId, \is_object($value) ? $class : \get_debug_type($value), $parameter);
        }
    }
    private function getExpressionLanguage() : \WPR_Vendor\Symfony\Component\DependencyInjection\ExpressionLanguage
    {
        if (null === $this->expressionLanguage) {
            $this->expressionLanguage = new \WPR_Vendor\Symfony\Component\DependencyInjection\ExpressionLanguage(null, $this->container->getExpressionLanguageProviders());
        }
