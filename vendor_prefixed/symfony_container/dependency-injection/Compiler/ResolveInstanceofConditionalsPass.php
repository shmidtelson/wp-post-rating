 ...
 */
class ResolveInstanceofConditionalsPass implements \WPR_Vendor\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    /**
     * {@inheritdoc}
     */
    public function process(\WPR_Vendor\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        foreach ($container->getAutoconfiguredInstanceof() as $interface => $definition) {
            if ($definition->getArguments()) {
                throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Autoconfigured instanceof for type "%s" defines arguments but these are not supported and should be removed.', $interface));
            }
        }
    ...
    private function processDefinition(\WPR_Vendor\Symfony\Component\DependencyInjection\ContainerBuilder $container, string $id, \WPR_Vendor\Symfony\Component\DependencyInjection\Definition $definition) : \WPR_Vendor\Symfony\Component\DependencyInjection\Definition
    {
        ...
        $parent = $definition instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\ChildDefinition ? $definition->getParent() : null;
        ...
        if ($parent) {
            ...
            if (\WPR_Vendor\Symfony\Component\DependencyInjection\Definition::class === \get_class($abstract)) {
    ...
    private function mergeConditionals(array $autoconfiguredInstanceof, array $instanceofConditionals, \WPR_Vendor\Symfony\Component\DependencyInjection\ContainerBuilder $container) : array
    {
        ...
        foreach ($instanceofConditionals as $interface => $instanceofDef) {
            ...
            if (!$container->getReflectionClass($interface)) {
                throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('"%s" is set as an "instanceof" conditional, but it does not exist.', $interface));
            }
