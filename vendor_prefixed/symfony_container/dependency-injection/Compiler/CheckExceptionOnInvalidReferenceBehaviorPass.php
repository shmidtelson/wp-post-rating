 ...
 */
class CheckExceptionOnInvalidReferenceBehaviorPass extends \WPR_Vendor\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    protected function processValue($value, bool $isRoot = \false)
    {
        ...
        if (\WPR_Vendor\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE < $value->getInvalidBehavior() || $this->container->has($id = (string) $value)) {
            return $value;
        }
        ...
        if (isset($this->serviceLocatorContextIds[$currentId])) {
            ...
            foreach ($locator->getArgument(0) as $k => $v) {
                if ($v->getValues()[0] === $value) {
                    ...
                    throw new \WPR_Vendor\Symfony\Component\DependencyInjection\Exception\ServiceNotFoundException($id, $currentId);
                }
            }
        }
        if ('.' === $currentId[0] && $graph->hasNode($currentId)) {
            foreach ($graph->getNode($currentId)->getInEdges() as $edge) {
                if (!$edge->getValue() instanceof \WPR_Vendor\Symfony\Component\DependencyInjection\Reference || \WPR_Vendor\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE < $edge->getValue()->getInvalidBehavior()) {
                    continue;
                }
