...
class Lexer
{
    ...
    const PUNCTUATION = '()[]{}?:.,|';
    public function __construct(\WPR_Vendor\Twig\Environment $env, array $options = [])
    {
        $this->env = $env;
    ...
    public function tokenize(\WPR_Vendor\Twig\Source $source) : \WPR_Vendor\Twig\TokenStream
    {
        ...
        if (!empty($this->brackets)) {
            ...
            throw new \WPR_Vendor\Twig\Error\SyntaxError(\sprintf('Unclosed "%s".', $expect), $lineno, $this->source);
        }
        return new \WPR_Vendor\Twig\TokenStream($this->tokens, $this->source);
    }
    ...
    private function lexExpression() : void
    {
        ...
        if (\preg_match('/\\s+/A', $this->code, $match, 0, $this->cursor)) {
            ...
            if ($this->cursor >= $this->end) {
                throw new \WPR_Vendor\Twig\Error\SyntaxError(\sprintf('Unclosed "%s".', self::STATE_BLOCK === $this->state ? 'block' : 'variable'), $this->currentVarBlockLine, $this->source);
            }
        }
        // arrow function
        if ('=' === $this->code[$this->cursor] && '>' === $this->code[$this->cursor + 1]) {
            $this->pushToken(\WPR_Vendor\Twig\Token::ARROW_TYPE, '=>');
            $this->moveCursor('=>');
            ...
            if (\false !== \strpos('([{', $this->code[$this->cursor])) {
                ...
                if (empty($this->brackets)) {
                    throw new \WPR_Vendor\Twig\Error\SyntaxError(\sprintf('Unexpected "%s".', $this->code[$this->cursor]), $this->lineno, $this->source);
                }
                list($expect, $lineno) = \array_pop($this->brackets);
            ...
            throw new \WPR_Vendor\Twig\Error\SyntaxError(\sprintf('Unexpected character "%s".', $this->code[$this->cursor]), $this->lineno, $this->source);
        }
    }
    private function lexRawData() : void
    {
        if (!\preg_match($this->regexes['lex_raw_data'], $this->code, $match, \PREG_OFFSET_CAPTURE, $this->cursor)) {
            throw new \WPR_Vendor\Twig\Error\SyntaxError('Unexpected end of file: Unclosed "verbatim" block.', $this->lineno, $this->source);
        }
    ...
    private function lexComment() : void
    {
        if (!\preg_match($this->regexes['lex_comment'], $this->code, $match, \PREG_OFFSET_CAPTURE, $this->cursor)) {
            throw new \WPR_Vendor\Twig\Error\SyntaxError('Unclosed comment.', $this->lineno, $this->source);
        }
    ...
    private function pushToken($type, $value = '') : void
    {
        ...
        $this->tokens[] = new \WPR_Vendor\Twig\Token($type, $value, $this->lineno);
    }
