 ...
 */
final class ModuleNode extends \WPR_Vendor\Twig\Node\Node
{
    public function __construct(\WPR_Vendor\Twig\Node\Node $body, ?\WPR_Vendor\Twig\Node\Expression\AbstractExpression $parent, \WPR_Vendor\Twig\Node\Node $blocks, \WPR_Vendor\Twig\Node\Node $macros, \WPR_Vendor\Twig\Node\Node $traits, $embeddedTemplates, \WPR_Vendor\Twig\Source $source)
    {
        $nodes = ['body' => $body, 'blocks' => $blocks, 'macros' => $macros, 'traits' => $traits, 'display_start' => new \WPR_Vendor\Twig\Node\Node(), 'display_end' => new \WPR_Vendor\Twig\Node\Node(), 'constructor_start' => new \WPR_Vendor\Twig\Node\Node(), 'constructor_end' => new \WPR_Vendor\Twig\Node\Node(), 'class_end' => new \WPR_Vendor\Twig\Node\Node()];
        if (null !== $parent) {
            $nodes['parent'] = $parent;
        }
    ...
    protected function compileTemplate(\WPR_Vendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileGetParent(\WPR_Vendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileClassHeader(\WPR_Vendor\Twig\Compiler $compiler)
    {
        $compiler->write("\n\nnamespace WPR_Vendor;\n\n");
        if (!$this->getAttribute('index')) {
            $compiler->write("use \\WPR_Vendor\\Twig\\Environment;\n")->write("use \\WPR_Vendor\\Twig\\Error\\LoaderError;\n")->write("use \\WPR_Vendor\\Twig\\Error\\RuntimeError;\n")->write("use \\WPR_Vendor\\Twig\\Extension\\SandboxExtension;\n")->write("use \\WPR_Vendor\\Twig\\Markup;\n")->write("use \\WPR_Vendor\\Twig\\Sandbox\\SecurityError;\n")->write("use \\WPR_Vendor\\Twig\\Sandbox\\SecurityNotAllowedTagError;\n")->write("use \\WPR_Vendor\\Twig\\Sandbox\\SecurityNotAllowedFilterError;\n")->write("use \\WPR_Vendor\\Twig\\Sandbox\\SecurityNotAllowedFunctionError;\n")->write("use \\WPR_Vendor\\Twig\\Source;\n")->write("use \\WPR_Vendor\\Twig\\Template;\n\n");
        }
    ...
    protected function compileConstructor(\WPR_Vendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileDisplay(\WPR_Vendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileClassFooter(\WPR_Vendor\Twig\Compiler $compiler)
    {
        $compiler->subcompile($this->getNode('class_end'))->outdent()->write("}\n");
    }
    protected function compileMacros(\WPR_Vendor\Twig\Compiler $compiler)
    {
        $compiler->subcompile($this->getNode('macros'));
    }
    protected function compileGetTemplateName(\WPR_Vendor\Twig\Compiler $compiler)
    {
        $compiler->write("public function getTemplateName()\n", "{\n")->indent()->write('return ')->repr($this->getSourceContext()->getName())->raw(";\n")->outdent()->write("}\n\n");
    }
    protected function compileIsTraitable(\WPR_Vendor\Twig\Compiler $compiler)
    {
        ...
        if ($traitable) {
            if ($this->getNode('body') instanceof \WPR_Vendor\Twig\Node\BodyNode) {
                $nodes = $this->getNode('body')->getNode(0);
            } else {
                $nodes = $this->getNode('body');
            }
            if (!\count($nodes)) {
                $nodes = new \WPR_Vendor\Twig\Node\Node([$nodes]);
            }
            foreach ($nodes as $node) {
                ...
                if ($node instanceof \WPR_Vendor\Twig\Node\BlockReferenceNode) {
                    continue;
                }
                $traitable = \false;
                break;
    ...
    protected function compileDebugInfo(\WPR_Vendor\Twig\Compiler $compiler)
    {
        $compiler->write("public function getDebugInfo()\n", "{\n")->indent()->write(\sprintf("return %s;\n", \str_replace("\n", '', \var_export(\array_reverse($compiler->getDebugInfo(), \true), \true))))->outdent()->write("}\n\n");
    }
    protected function compileGetSourceContext(\WPR_Vendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileLoadTemplate(\WPR_Vendor\Twig\Compiler $compiler, $node, $var)
    {
